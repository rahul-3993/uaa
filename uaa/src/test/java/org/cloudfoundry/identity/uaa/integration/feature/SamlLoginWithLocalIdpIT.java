...
public class SamlLoginWithLocalIdpIT {
    ...
    public void cleanup() {
        String token = IntegrationTestUtils.getClientCredentialsToken(baseUrl, "admin", "adminsecret");
        ScimGroup group = IntegrationTestUtils.getGroup(token, "", baseUrl, "zones.testzone1.admin");
        if (group != null) {
            IntegrationTestUtils.deleteGroup(token, "", baseUrl, group.getId());
        }
        group = IntegrationTestUtils.getGroup(token, "", baseUrl, "zones.testzone2.admin");
        if (group != null) {
            IntegrationTestUtils.deleteGroup(token, "", baseUrl, group.getId());
        }
        group = IntegrationTestUtils.getGroup(token, "", baseUrl, "zones.uaa.admin");
        if (group != null) {
            IntegrationTestUtils.deleteGroup(token, "", baseUrl, group.getId());
        }
    }
    ...
    public void testWebSSOProfileWithRedirectPostSamlIntegration() throws Exception {
        ...
        IdentityZoneConfiguration config = new IdentityZoneConfiguration();
        config.getSamlConfig().setSignatureAlgorithm(SHA256);
        IdentityZone spZone = IntegrationTestUtils.createZoneOrUpdateSubdomain(identityClient, baseUrl, spZoneId, spZoneId, config);
